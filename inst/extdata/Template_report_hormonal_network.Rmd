---
title: "Global Report with Hormonal Network Analysis"
output:
  html_document:
    toc: true
    toc_depth: 2
...
vignette: >
  %\VignetteIndexEntry{Template_report_hormonal}
...
**Includes:** - Differential expression analysis (DEGs) - Pathway enrichment analysis - Hormonal protein-protein interaction networks analysis
...
# Hormonal protein-protein interaction network (PPI)
  ...
  for (j in seq_along(sublist)) {
    ...
    if (vcount(g) > 2) {
      ...
      gplots[[inner_name]] <- ggraph(layout) +
        ...
        theme_void() +
        labs(
          title = paste("Hormonal protein-protein interaction network -", outer_name, "-", inner_name),
          fill = "Node Degree"
        ) +
